...
The search finds that the two free embeddings settle almost exactly antipodal to the pinned ones. The $(f_1, f_3)$ pair is opened by $11.0^\circ$ with a length ratio $|f_3| / |f_1| = 2.35$, and the $(f_2, f_4)$ pair by $9.4^\circ$ with $|f_4| / |f_2| = 2.82$, reaching an MSE floor of $0.00243$ per feature, against $0.0059$ for the closed symmetric pairs. The opening angle itself is a resolution effect. A closed-form solution tells us that the cost keeps decreasing as the angle between the feature pair approaches 0, without ever reaching it, so an ideal decoder would open them by an arbitrarily small angle. However, a decoder of finite sharpness (a binned one, or a trained network) settles on a finite opening.
...
![The geometry found by the search: the pair $(f_1, f_3)$ opened by $11.0^\circ$ at length ratio $2.35$, the pair $(f_2, f_4)$ by $9.4^\circ$ at $2.82$. Each shaded parallelogram is the region where the readings of two co-active features land; the two thin orange slivers belong to the pairs.](figures/optimal_geometry.png)
